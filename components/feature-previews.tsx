...
const previewNotes = seedNotes("preview-course-0", "preview-course-1")
  .slice(0, 3)
  .map((note, idx) => ({
    ...note,
    id: `preview-note-${idx}`,
...
const previewDailyEntries = getSeedDailyEntries()
  .slice(0, 3)
  .map((entry, idx) => ({
    ...entry,
...
const previewChartNotes = seedNotes("preview-course-0", "preview-course-1").map(
  (note, idx) => ({
    ...
    <div className="px-3 py-0.5 pointer-events-none h-full overflow-hidden relative [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div className="flex animate-marquee-right gap-3 h-full items-center whitespace-nowrap">
        {duplicatedCourses.map((course, idx) => (
          <Card
            key={`${course.id}-${idx}`}
            className="border shadow-sm flex-shrink-0 w-[200px] inline-block"
          >
            <CardHeader className="pb-1 pt-1.5 px-2.5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h2 className="text-sm font-semibold leading-tight mb-0.5 line-clamp-1">
                    {course.title}
                  </h2>
...
export function NotesPreview() {
  ...
  const allNotes = seedNotes("preview-course-0", "preview-course-1")
    .slice(0, 4)
    .map((note, idx) => ({
      ...note,
      id: `preview-note-${idx}`,
        ...
        {duplicatedNotes.map((note, idx) => (
          <Card key={`${note.id}-${idx}`} className="border shadow-sm">
            <CardHeader className="pb-1 pt-1.5 px-2.5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h2 className="text-xs font-semibold leading-tight mb-0.5 line-clamp-1">
                    {note.question}
                  </h2>
                  <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                    ...
                    {note.flag && (
                      <Badge
                        variant="outline"
                        className="text-orange-600 border-orange-600 text-[10px] px-1.5 py-0"
                      >
...
export function DailyPreview() {
  ...
  const allEntries = getSeedDailyEntries()
    .slice(0, 4)
    .map((entry, idx) => ({
      ...entry,
      id: `preview-entry-${idx}`,
...
export function ChartsPreview() {
  ...
  return (
    <div className="px-4 pt-2 pb-3 pointer-events-none overflow-hidden h-full flex items-start [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div
        className={`w-full h-[400px] min-h-[400px] scale-70 origin-top transition-opacity duration-500 ease-in-out ${
          isTransitioning ? "opacity-0" : "opacity-100"
        }`}
        key={chartIndex}
      >
